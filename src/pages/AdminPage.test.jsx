...
describe('AdminPage', () => {
  ...
  describe('Owner-bruger (fuld adgang)', () => {
    ...
    it('viser alle elleve faner for owner (inkl. Indstillinger)', () => {
      renderAdminPage();
      const tabs = screen.queryAllByTestId(/^tab-/);
      expect(tabs).toHaveLength(11);
      expect(screen.queryByTestId('tab-altstanding')).toBeInTheDocument();
      expect(screen.queryByTestId('tab-groupwinners')).toBeInTheDocument();
  ...
  describe('Global admin (ikke ejer)', () => {
    ...
    it('viser præcis 10 faner for global admin (ingen Indstillinger)', () => {
      renderAdminPage();
      const tabs = screen.queryAllByTestId(/^tab-/);
